// Country dial codes for the phone field. iso = ISO 3166-1 alpha-2 (used as a
// stable <option> value and to render a flag emoji); dial = E.164 calling code.
...
/** Splits a stored phone ("+91 98765 43210") into a country + local number. */
export function splitPhone(
  ...
  if (m) {
    // Longest dial code that prefixes the number wins (e.g. +1268 over +1).
    const match = [...COUNTRIES]
      .sort((a, b) => b.dial.length - a.dial.length)
      .find((c) => m[1] === c.dial || raw.startsWith(c.dial));
    if (match) {
      const rest = raw.slice(match.dial.length).trim();
      return { iso: match.iso, number: rest };
    }
    return { iso: fallbackIso, number: m[2] };
  }
  return { iso: fallbackIso, number: raw };
}
